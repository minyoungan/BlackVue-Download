...
for file in `curl http://$DASHCAM_IP/blackvue_vod.cgi | sed 's/^n://' | sed 's/F.mp4//' | sed 's/R.mp4//' | sed 's/,s:1000000//' | sed $'s/\r//' | grep $TODAY`; do
	...
	if [ -f $VIDEONAME\F.mp4 ]; then
		sqlite3 /mnt/PASSPORT/tesla.db -cmd ".timeout 5000" "INSERT OR IGNORE INTO videos(VideoName,CameraSide) VALUES('$VIDEONAME','B');"
	fi

	# Left Video
	wget -c http://$DASHCAM_IP$file\R.mp4;
	if [ -f $VIDEONAME\R.mp4 ]; then
		sqlite3 /mnt/PASSPORT/tesla.db -cmd ".timeout 5000" "INSERT OR IGNORE INTO videos(VideoName,CameraSide) VALUES('$VIDEONAME','R');"
	fi
